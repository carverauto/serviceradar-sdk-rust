...
## 3. Result Payloads
- [x] 3.1 Implement `serviceradar.plugin_result.v1` types, including status, labels, display widgets, events, alert hints, and condition IDs; time-series metrics are emitted through first-class `serviceradar.metric.v1.MetricBatch` telemetry, not plugin-result fields.
